...
public class TokenProviderTest {
    ...
    void generateToken() {
        ...
        // when : 토큰 제공자의 generateToken() 메서드를 호출해 토큰 만듦
        String token = tokenProvider.generateToken(testUser, Duration.ofDays(14));

        // then : jjwt 라이브러리를 사용해 토큰 복호화. 토큰을 만들 때 클레임으로 넣어둔
        // id 값이 given 절에서 만든 유저 ID와 동일한지 확인
    ...
    void validToken_invalidToken() {
        ...
        // 이때, 만료 시간은 1970년 1월 1일부터 현재 시간을 밀리초 단위로 치환한 값에 1000을 빼, 이미 만료된 토큰으로 생성
    ...
    void getAuthentication() {
        // given : jjwt 라이브러리를 사용해 토큰 생성. 토큰의 제목인 subject 는 user@email.com
    ...
    @Test
    void getUserId() {
        // given : jjwt 라이브러리를 사용해 토큰 생성. 이때 클레임을 추가.
        Long userId = 1L;
        String token = JwtFactory.builder()
        ...
        // when : 토큰 제공자의 메서드를 호출해 유저 ID를 반환
        Long userIdByToken = tokenProvider.getUserId(token);

        // then : 반환받은 유저 ID가 given 절에서 설정한 유저 id 값과 같은지 확인
